...
public class ItemBlockEnderInterface extends ItemBlock{

	public ItemBlockEnderInterface(Block block) {
		super(block);
		setMaxStackSize(1);
	}
	...
	public boolean onItemUse(ItemStack itemstack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
		if(!hasSyncData(itemstack) || player.isSneaking()){
			...
			player.addChatComponentMessage(new ChatComponentText("Succesfully linked ender interface"));
			return true;
	...
	public boolean placeBlockAt(ItemStack itemstack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ, int metadata) {
		if(super.placeBlockAt(itemstack, player, world, x, y, z, side, hitX, hitY, hitZ, metadata)){
			...
			TileEntityEnderInterface tileentity = (TileEntityEnderInterface) world.getTileEntity(x, y, z);
