...
public class BlockInterface extends BlockContainer {
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float f, float g,
		...
		if (ID > 0
				&& ID < Block.blocksList.length
				&& Block.blocksList[ID] != null
				&& (!(Block.blocksList[ID] instanceof BlockInterface) || direct != ForgeDirection.getOrientation(
						world.getBlockMetadata(x + direct.offsetX, y + direct.offsetY, z + direct.offsetZ))
						.getOpposite()))
			return Block.blocksList[ID].onBlockActivated(world, x + direct.offsetX, y + direct.offsetY, z
					+ direct.offsetZ, player, side, f, g, t);
		else
			return false;
	...
	public boolean rotateBlock(World world, int x, int y, int z, ForgeDirection side) {
		...
		world.setBlock(x, y, z, blockID, meta.getRotation(side).ordinal(), 2);
